...
const ButtonItem = forwardRef<HTMLButtonElement, ButtonProps>(({ onClick: handleClick, children, value, valueAsDescription = false, isLoading = false, disabled = false, className, variant = 'default', rightIcon, ...rest }, ref) => {
	...
	return (
		<button
			className={ cn(
				'group flex justify-between items-center border border-slate-200 first:rounded-t-md last:rounded-b-md text-sm py-2 px-4 hover:cursor-default [&:not(:first-child)]:-mt-[1px] disabled:pointer-event-none disabled:cursor-default disabled:hover:!bg-transparent',
				{
